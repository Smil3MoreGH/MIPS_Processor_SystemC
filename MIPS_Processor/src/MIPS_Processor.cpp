...
void MIPS_Processor::Behavioral()
{
    ...
    sc_lv<16> pc_current, pc_next, pc2, instr;

    sc_lv<2> reg_dst, mem_to_reg, alu_op;

    sc_signal <bool> jump, branch, mem_read, mem_write, alu_src, reg_write;

    sc_lv<3> reg_write_dest;
    sc_lv<16> reg_write_data;

    sc_lv<3> reg_read_addr_1;
    sc_lv<16> reg_read_data_1;

    sc_lv<3> reg_read_addr_2;
    sc_lv<16> reg_read_data_2;

    sc_lv<3> ALU_Control;
    sc_lv<16> ALU_out;

    sc_lv<16> sign_ext_im, read_data2, zero_ext_im, imm_ext;

    sc_signal <bool> zero_flag, JRControl, beq_control;

    sc_lv<16> im_shift_1, PC_j, PC_beq, PC_4beq, PC_4beqj, PC_jr;

    sc_lv<15> jump_shift_1;
    sc_lv<16> mem_read_data;
    sc_lv<16> no_sign_ext;
    ...
    pc2 = pc_current + "0000000000000010";
    ...
    if (reg_dst.read().to_uint() == 10)
    {
        reg_write_dest.write(111);
    }
    else if(reg_dst.read() == 01)
    {
    ...
    if(alu_op.read().to_uint() == 0 && instr.read().range(3,0) == 1000)
    {
        JRControl.write(1);
    }
    else
    ...
    no_sign_ext.write(!im_shift_1 + 0x0001);
...
int sc_main(int argc, char* argv[])
{
	...
	sc_trace(trace_file , pc_out, "pc_out");
